 ...
 */
public class FlatCheckBox extends CheckBox implements
	...
	private void init(AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray a = getContext().obtainStyledAttributes(attrs,
					R.styleable.fl_FlatCheckBox);

			// getting common attributes
			int customTheme = a.getResourceId(
					R.styleable.fl_FlatCheckBox_fl_theme,
					Attributes.DEFAULT_THEME);
			attributes.setThemeSilent(customTheme, getResources());

			attributes.setFontFamily(a
					.getString(R.styleable.fl_FlatCheckBox_fl_fontFamily));
			attributes.setFontWeight(a
					.getString(R.styleable.fl_FlatCheckBox_fl_fontWeight));
			attributes.setFontExtension(a
					.getString(R.styleable.fl_FlatCheckBox_fl_fontExtension));
			...
			attributes.setRadius(a.getDimensionPixelSize(
					R.styleable.fl_FlatCheckBox_fl_cornerRadius,
					Attributes.DEFAULT_RADIUS_PX));
			attributes.setBorderWidth(attributes.getSize() / 10);

			// getting view specific attributes
			dotMargin = a.getDimensionPixelSize(
					R.styleable.fl_FlatCheckBox_fl_dotMargin, dotMargin);
		...
		uncheckedEnabled.setStroke(attributes.getBorderWidth(),
		...
		checkedOutside.setStroke(attributes.getBorderWidth(),
				attributes.getColor(2));
		...
		InsetDrawable checkedInside = new InsetDrawable(checkedCore,
				attributes.getBorderWidth() + dotMargin,
				attributes.getBorderWidth() + dotMargin,
				attributes.getBorderWidth() + dotMargin,
				attributes.getBorderWidth() + dotMargin);
		...
		uncheckedDisabled.setStroke(attributes.getBorderWidth(),
				attributes.getColor(3));
		...
		checkedOutsideDisabled.setSize(attributes.getSize(),
				attributes.getSize());
		checkedOutsideDisabled.setColor(Color.TRANSPARENT);
		checkedOutsideDisabled.setStroke(attributes.getBorderWidth(),
				attributes.getColor(3));
		...
		InsetDrawable checkedInsideDisabled = new InsetDrawable(
				checkedCoreDisabled, attributes.getBorderWidth() + dotMargin,
				attributes.getBorderWidth() + dotMargin,
				attributes.getBorderWidth() + dotMargin,
				attributes.getBorderWidth() + dotMargin);

		Drawable[] checkedDisabledDrawable = { checkedOutsideDisabled,
				checkedInsideDisabled };
